	...
	try:
		...
		if isSSProject(sequence):
			...
			for contig in contigRecords:
				writeFile.write(contig.format("genbank"))  # Write each contig to the same file in fasta format.
		# If accession is a regular genome... 
		else:	
			writeFile.write(sequence.format("genbank"))  # Write genome as fasta to file
		writeFile.close()
	except IOError:
