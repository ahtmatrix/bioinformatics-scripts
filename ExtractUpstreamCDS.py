...
def validate_cds(record, feature):
    ...
    try:
        protein_in_file = str(feature.qualifiers.get(
            'translation', 'no_translation')).strip('\'[]')
        # diff extracted CDS compare with FASTA nucleotdie on NCBI

        # is the problem with
        cds_to_protein = str(feature.extract(
...
def get_upstream_cds(fullpath, filename):
    ...
    SeqIO.write(extracted_cds_list, filename + "_" +
                str(num_bp_upstreamcds) + "upstream_" + ".CDS.fasta", "fasta")
    return
